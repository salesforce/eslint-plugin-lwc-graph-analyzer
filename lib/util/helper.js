...
/**
 * Sets the `LwcBundle` object in the in-memory cache.
 *
 * @param {LwcBundle | null} inputLwcBundle - The `LwcBundle` to set, or `null` to clear the cache.
 */
function setLwcBundleCache(inputLwcBundle) {
    lwcBundle = inputLwcBundle;
}

function analyzeLWC(context) {
    const eslintReports = getKomaciReport(context.id);
...
function getKomaciReport(ruleName) {
    if (!lwcBundle) {
